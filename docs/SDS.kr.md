...
- **SDS-ARCH-XXX**: Architectural Design (아키텍처 설계)
- **SDS-MOD-XXX**: Module Design (모듈 설계)
- **SDS-CLS-XXX**: Class Design (클래스 설계)
- **SDS-IF-XXX**: Interface Design (인터페이스 설계)
- **SDS-DATA-XXX**: Data Design (데이터 설계)
- **SDS-SEQ-XXX**: Sequence Design (시퀀스 설계)
...
본 문서는 DICOM Viewer 소프트웨어의 상세 설계 명세서(SDS)입니다. SRS에서 정의된 소프트웨어 요구사항을 구현하기 위한 아키텍처, 모듈, 클래스, 인터페이스 및 데이터 설계를 제공합니다.

### 1.2 Scope

이 문서는 다음 설계 측면을 다룹니다:
- 시스템 아키텍처 설계
- 모듈별 상세 설계
- 클래스 다이어그램 및 관계
- 인터페이스 정의
...
| 원칙 | 설명 | 적용 |
|------|------|------|
| **Separation of Concerns** | 관심사 분리 | 레이어 아키텍처, MVC 패턴 |
| **Dependency Injection** | 의존성 주입 | 서비스 레이어 |
| **Interface Segregation** | 인터페이스 분리 | 작은 단위 인터페이스 |
| **Open/Closed** | 개방-폐쇄 원칙 | 확장 가능한 플러그인 구조 |
| **Single Responsibility** | 단일 책임 | 클래스별 명확한 역할 |
...
> **Note**: 다이어그램은 **Mermaid 형식**(GitHub/GitLab 자동 렌더링)과 **ASCII 형식**(범용 호환성)을 모두 제공합니다.
...
|-------|---------------|------------------|--------------|
| **Presentation** | UI 렌더링, 사용자 입력 처리 | Qt6, QVTKOpenGLNativeWidget | Controller |
| **Controller** | 요청 조율, 이벤트 핸들링 | C++ | Service |
| **Service** | 비즈니스 로직, 영상 처리 | ITK, VTK | Data |
| **Data** | 데이터 저장, 변환, 관리 | pacs_system | External Libs |
| **External Libs** | 기반 기능 제공 | ITK, VTK, Qt6, pacs_system | OS |
...
**Purpose**: 플랫폼 독립적 의료 영상 뷰잉을 위한 서버 사이드 렌더링 + 이미지 스트리밍 아키텍처 옵션

> **Note**: 이 아키텍처는 웹, iOS, Android, 데스크톱 등 다양한 플랫폼에서 GPU 없이도 의료 영상을 볼 수 있게 하는 대안적 접근 방식입니다. 상세 구현은 [REF-007: Remote Visualization](reference/07-remote-visualization.md)을 참조하세요.

#### Architecture Comparison

| Aspect | Desktop Architecture (Default) | Remote Visualization Architecture |
|--------|-------------------------------|-----------------------------------|
| **렌더링 위치** | 클라이언트 (로컬 GPU) | 서버 (중앙 GPU) |
| **데이터 전송** | DICOM 파일 다운로드 | 이미지 스트림 (JPEG/H.264) |
| **클라이언트 요구사항** | GPU, VTK 라이브러리 | 웹 브라우저 또는 경량 앱 |
| **대역폭 사용** | 높음 (초기 다운로드) | 중간 (지속적 스트리밍) |
| **보안** | 로컬 데이터 저장 | 서버에 PHI 유지 (HIPAA 친화적) |
| **확장성** | 클라이언트별 독립 | GPU 클러스터 공유 |
...
|-----------|----------------|------------|
| **RenderServer** | 오프스크린 VTK 렌더링, 카메라 조작 | VTK, vtkRenderWindow |
| **StreamingServer** | WebSocket 통신, 세션 관리, 프레임 스트리밍 | WebSocket++, nlohmann/json |
| **AdaptiveQualityManager** | 네트워크 상태에 따른 품질 조정 | Custom |
| **ImageEncoder** | JPEG/WebP/H.264 인코딩 | libjpeg, libwebp, NVENC |

#### When to Use Remote Visualization

| Use Case | Recommended Architecture |
|----------|-------------------------|
| 고성능 워크스테이션 진단 | Desktop (Default) |
| 원격 진료 / 협업 | Remote Visualization |
| 모바일 참조 | Remote Visualization |
| 대용량 데이터셋 (>1GB) | Remote Visualization |
| 오프라인 환경 | Desktop (Default) |
| 웹 기반 PACS 통합 | Remote Visualization |

#### Implementation Reference

상세 구현 코드, 클라이언트 예제 (React, Flutter), 배포 설정 (Docker, Kubernetes)은 다음 문서를 참조하세요:

- **[REF-007: Remote Visualization Architecture](reference/07-remote-visualization.md)** - 전체 아키텍처 및 구현 가이드
...
**Purpose**: 공통 데이터 구조, 유틸리티, 타입 정의 제공
...
**Purpose**: DICOM 로딩, 전처리, 분할, 변환 기능 제공
...
**Purpose**: 볼륨 렌더링, 표면 렌더링, MPR, 2D 뷰 제공
...
**Purpose**: 거리, 각도, 면적, 볼륨 측정 및 ROI 관리
...
**Purpose**: PACS 연동 (C-FIND, C-MOVE, C-STORE, C-ECHO)
...
sequenceDiagram
    ...
    Note over DL,IB: HU 변환: StoredValue × RescaleSlope + RescaleIntercept
```
...
sequenceDiagram
    ...
    Note over SG: ITK ConnectedThreshold<br/>또는 NeighborhoodConnected
...
sequenceDiagram
    ...
    Note over SC: 통계 계산:<br/>Mean, StdDev, Min, Max,<br/>Percentiles
...
| FR-004.1 | SRS-FR-012 | SDS-MOD-003 | RenderService | Planned |
| FR-004.2 | SRS-FR-012 | SDS-MOD-003 | RenderService | Planned |
| FR-004.3 | SRS-FR-013 | SDS-MOD-003 | RenderService | Planned |
| FR-004.4 | SRS-FR-014 | SDS-MOD-003 | RenderService | Planned |
| FR-004.5 | SRS-FR-015 | SDS-MOD-003 | RenderService | Planned |
...
| FR-007.1~5 | SRS-FR-036~038 | SDS-MOD-004, SDS-SEQ-003 | MeasurementService | Planned |
